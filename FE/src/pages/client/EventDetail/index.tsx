...
import { Typography, Button, message, Progress, Avatar, Rate } from "antd"; // Import thêm Avatar, Rate
import {
  CalendarOutlined,
  EnvironmentOutlined,
  CommentOutlined,
...
const EventDetail: React.FC = () => {
  ...
  const [pendingTicket, setPendingTicket] =
    React.useState<TicketDisplay | null>(null);
  ...
  React.useEffect(() => {
    if (!event) return;
    const trySetOrganizer = async () => {
      try {
        // nếu backend trả organizer object đã populate
        if (
          event.organizer &&
          typeof event.organizer === "object" &&
          (event.organizer.name || event.organizer._id)
        ) {
        ...
        const res = await axiosClient.get(`/api/organizer/${organizerId}`);
        setOrganizer(res.data);
      } catch (err) {
        console.error("Fetch organizer failed", err);
      }
    };
    trySetOrganizer();
  }, [event]);
  ...
  React.useEffect(() => {
    ...
    const fetchTickets = async () => {
      try {
        const ticketClasses: TicketClass[] = await getTicketClassesByEventAPI(
          event._id,
        );
      ...
      <div className="container mx-auto px-6 py-8 bg-[#1d3f73] ">
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <Title
              level={3}
              className="!text-[#23A6F0] !m-0 border-l-4 border-[#23A6F0] pl-3"
            >
              GIỚI THIỆU
            </Title>
            <button
        ...
        <section className="mb-8" ref={priceSectionRef}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <Title level={3} className="!text-[#23A6F0] !m-0">
              CHỌN LOẠI VÉ
            </Title>
          </div>
                ...
                {ticketsDisplay.length > 0 ? (
                  ticketsDisplay.map((t, i) => {
                    const percentSold =
                      t.totalQty > 0
                        ? Math.round(
                            ((t.totalQty - t.remaining) / t.totalQty) * 100,
                          )
                        : 100;
                      ...
                      >
                        <td className="py-4 px-6">
                          ...
                          {t.isReserved ? (
                            <div className="text-xs text-blue-500 mt-1 font-medium">
                              Vé ngồi cố định (Chọn ghế ở bước sau)
                            </div>
                          ) : (
                            <div className="text-xs text-gray-500 mt-1">
                              Vé tự do trong khu vực
                            </div>
                          )}
                        </td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6">
                          <span className="text-[#E04646] font-bold text-lg">
                            {parseInt(String(t.price)).toLocaleString("vi-VN")}{" "}
                            đ
                          </span>
                        </td>
        ...
        <LoginModal
          ...
          onLoginSuccess={() => {
            ...
            if (pendingTicket) {
              navigate("/checkout", {
                state: { event, ticketInfo: { ...pendingTicket, seat: null } },
              });
        ...
        {/* Organizer card (moved to bottom) */}
        {organizer && (
          <section className="mt-8">
            ...
            <div className="bg-white p-4 rounded-lg border text-gray-800 border-gray-100 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 text-sm mt-2 mb-2 text-gray-600">
                  {organizer.phoneNumber && (
                    <span className="flex items-center gap-1">
                      <PhoneOutlined /> {organizer.phoneNumber}
                    </span>
                  )}
                  {organizer.email && (
                    <span className="flex items-center gap-1">
                      <MailOutlined /> {organizer.email}
                    </span>
                  )}
                </div>
                <Rate
